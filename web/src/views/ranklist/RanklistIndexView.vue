<template>
    <ContentFieldVue>
        <table class="table table-striped table-hover" style="text-align:center">
            <thead>
                <tr>
                    <th>玩家</th>
                    <th>天梯积分</th>
                    <th>操作</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="user in users" :key="user.id" style="line-height:28px;">
                    <td>
                        <img :src="user.photo" alt="" class="ranklist-user-photo">
                        &nbsp;
                        <span class="ranklist-user-username"> {{ user.username }}</span>
                    </td>
                    <td>
                        {{ user.rating }}
                    </td>
                    <td>
                        <button type="button"  class="btn btn-secondary btn-sm"
                            @click="changeFollow(user.id, user.follow)"
                            v-if="user.id != $store.state.user.id"
                            >
                            {{ user.follow === true ? '取消关注' : '+关注' }}
                        </button>
                    </td>
                </tr>
            </tbody>
        </table>
        <nav aria-label="...">
            <ul class="pagination" style="float:right">
                <li class="page-item" @click="click_page(-2)">
                    <a class="page-link" href="#">前一页</a>
                </li>
                <li :class="'page-item ' + page.is_active" v-for="page in pages" :key="page.pageId"
                    @click="click_page(page.pageId)">
                    <a class="page-link" href="#">{{ page.pageId }}</a>
                </li>
                <li class="page-item" @click="click_page(-1)">
                    <a class="page-link" href="#">后一页</a>
                </li>
            </ul>
        </nav>
    </ContentFieldVue>
</template>
<script>
import ContentFieldVue from '../../components/ContentField.vue'
import { useStore } from 'vuex';
import $ from 'jquery'
import { ref } from 'vue';

export default {
    components: {
        ContentFieldVue
    },
    setup() {
        const store = useStore();
        let current_page = 1;
        let users = ref([]);
        let total_users = 0;
        let pages = ref([]);

        const click_page = page => {
            if (page === -2)
                page = current_page - 1;
            if (page === -1)
                page = current_page + 1;
            let max_pages = parseInt(Math.ceil(total_users / 10));
            if (page >= 1 && page <= max_pages) {
                pull_page(page);//加载一个新的分页
            }
        }
        const update_pages = () => {
            let max_pages = parseInt(Math.ceil(total_users / 10));//最大页数
            let new_pages = [];
            for (let i = current_page - 2; i <= current_page + 2; i++) {
                if (i >= 1 && i <= max_pages) {
                    new_pages.push({
                        pageId: i,
                        is_active: i === current_page ? "active" : ""
                    });
                }
            }
            pages.value = new_pages;
        }

        const pull_page = page => {
            current_page = page;
            $.ajax({
                url: "http://127.0.0.1:3000/ranklist/getlist/",
                data: {
                    page,
                },
                type: "get",
                headers: {
                    Authorization: "Bearer " + store.state.user.token,
                },
                success(resp) {
                    users.value = resp.users;
                    total_users = resp.users_count;
                    update_pages();
                    getFollow();
                },
                error(resp) {
                    console.log(resp)
                }
            })
        }
        pull_page(current_page);
        const changeFollow = (followeeId, follow) => {
            if (store.state.user.id == followeeId) {
                return;
            }
            $.ajax({
                url: "http://127.0.0.1:3000/ranklist/follow/",
                data: {
                    followeeId,
                    flag: follow ? -1 : 1,
                },
                type: "post",
                headers: {
                    Authorization: "Bearer " + store.state.user.token,
                },
                success(resp) {
                    getFollow()
                    console.log(resp, "follow")
                },
                error(resp) {
                    console.log(resp)
                }
            })
        }
        const getFollow = () => {
            $.ajax({
                url: "http://127.0.0.1:3000/ranklist/follow/getlist/",
                data: {
                    type: "1"
                },
                type: "get",
                headers: {
                    Authorization: "Bearer " + store.state.user.token,
                },
                success(resp) {
                    users.value.forEach(user => {
                        user.follow = null;
                        const hasMatch = resp.some(response => response.followeeId === user.id);
                        if (hasMatch) {
                            user.follow = true
                        }
                    });
                },
                error(resp) {
                    console.log(resp)
                }
            })
        }
        getFollow()
        return {
            users,
            total_users,
            pages,
            click_page,
            changeFollow,
            getFollow
        }
    }
}
</script>
<style scoped>
img.ranklist-user-photo {
    width: 4vh;
    border-radius: 50%;
}
</style>